...
# ✅ Fix 72-byte bcrypt limit: truncate bytes
def _bcrypt_compat(pw: str) -> bytes:
    b = pw.encode("utf-8")
    return b[:72] if len(b) > 72 else b


def hash_pw(pw: str) -> str:
    return pwd_context.hash(_bcrypt_compat(pw))


def verify_pw(pw: str, pw_hash: str) -> bool:
    return pwd_context.verify(_bcrypt_compat(pw), pw_hash)
